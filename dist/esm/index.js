...
class FirebaseEditor {
    ...
    selections;
    view;
    constructor({ firebaseRef, stateConfig, view: constructView, clientID: selfClientID = firebaseRef.push().key, progress = () => { }, }) {
        progress(0 / 2);
        ...
        const selfChanges = {};
        let selection;
        const constructEditor = checkpointRef.once('value').then((snapshot) => {
            progress(1 / 2);
                ...
                if (sendable) {
                    ...
                    (existingBatchedSteps) => {
                        if (!existingBatchedSteps) {
                            ...
                            return {
                                s: compressStepsLossy(steps).map((step) => compressStepJSON(step.toJSON())),
                                c: clientID,
                                t: TIMESTAMP,
                            };
                        }
            ...
            (snapshot) => {
                ...
                (snapshot) => {
                    ...
                    const steps = (clientID === selfClientID
